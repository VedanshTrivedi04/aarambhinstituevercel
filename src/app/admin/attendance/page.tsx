...
export default function AdminAttendancePage() {
  ...
  useEffect(() => {
    authFetch<Batch[]>("/api/v1/admin/academic/batches").then((res) => {
      if (res.data) {
        ...
        if ((res.data as any).length > 0) setSelectedBatch((res.data as any)[0].id);
      }
    });
  }, []);
  ...
  const loadRoster = useCallback(async () => {
    ...
    // 1. Find (or create) today's session for this batch
    ...
    // 2. Load the roster/sheet for that session
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            Log daily student presence per session — synced directly to the attendance ledger.
          </p>
        </div>
