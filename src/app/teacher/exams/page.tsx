...
export default function TeacherExamsPage() {
  ...
  const loadTestsForBatches = useCallback(async (batchList: Batch[]) => {
    ...
    if (res.data) {
      const items = ((res.data as any).items as TestRead[]).filter(
        (t) => t.batch_id && batchIds.has(t.batch_id)
      );
      setTests(items);
      if (items.length > 0) setSelectedTestId(items[0].id);
    } else if (res.error) {
      setError(res.error);
    }
  ...
  useEffect(() => {
    (async () => {
      const meRes = await authFetch<{ data: { profile: { id: string } } }>("/api/v1/me/profile");
      const teacherId = (meRes.data as any)?.data?.profile?.id;
      if (!teacherId) {
        setError("Could not resolve your teacher profile.");
        return;
      }
      const batchRes = await authFetch<Batch[]>(`/api/v1/admin/academic/batches?teacher_id=${teacherId}`);
      const myBatches = (batchRes.data as any) || [];
      setBatches(myBatches);
      loadTestsForBatches(myBatches);
    })();
  }, [loadTestsForBatches]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-xs font-bold text-slate-500 mb-1">
            <span>Faculty</span>
            <span>/</span>
            <span className="text-[#c22329]">Exams & Marks Entry</span>
          </div>
          <h1 className="text-2xl sm:text-3xl font-black text-slate-900">
            My Tests & Marks Ledger
          </h1>
          <p className="text-xs text-slate-500 mt-0.5">
            Enter marks for your own batches — ranks and percentiles are computed automatically on save.
          </p>
        </div>
      ...
      {loadingTests ? (
        ...
        <div className="bg-white border border-slate-200 rounded-3xl p-8 text-center text-sm text-slate-500">
          No tests scheduled for your batches yet. Click "+ Schedule Test" to create one.
        </div>
      ) : (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white border border-slate-200 rounded-3xl p-6 sm:p-8 max-w-lg w-full space-y-5 shadow-2xl relative text-slate-900">
            ...
            <form onSubmit={handleCreateTest} className="space-y-4 text-xs">
              ...
              <div className="space-y-1">
                <label className="font-bold text-slate-700">My Batch *</label>
                <select
                  required
                  value={form.batch_id}
